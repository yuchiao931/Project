...
# 1) Pull pop and merge
wb_pop_ts <- WDI("all", "SP.POP.TOTL", 1990, 2019, extra=TRUE) %>%
...
# 2) Compute scale factor to align hom_w with migr_w scale
sf <- max(ts_df$hom_w) / max(ts_df$migr_w)

# 3) Plot: migration on primary axis, homicide/sf on primary
...
![Figure 3. Problem 2](Fig/problem 2.png)
...
df_sc <- df2 %>%
  left_join(wb_pop, by = c("code","year")) %>%
  select(code, year, ln_migr, ln_homic, pop_total) %>%  # ← Throw away population, homicide_rate, and migr_pop
  pivot_wider(
    names_from  = year,
    values_from = c(ln_migr, ln_homic, pop_total),
    names_sep   = "_"
